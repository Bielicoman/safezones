...
(function () {
  ...
  function initSimulator() {
    ...
    /* Bind Ratio Toggles */
    var ratioBtns = document.querySelectorAll('#sim-ratio-toggles .sim-btn');
    ...
    /* Bind Social Toggles */
    var socialBtns = document.querySelectorAll('#sim-social-toggles .sim-btn');
    ...
    /* Bind Colors */
    var colorBtns = document.querySelectorAll('#sim-color-swatches .sim-color-btn');
    ...
    if (inGuideAlpha) {
      inGuideAlpha.addEventListener('input', function () {
        ...
        document.getElementById('sim-guide-alpha-val').textContent = this.value + '%';
        renderSim();
      });
    }
    ...
    if (inArtAlpha) {
      inArtAlpha.addEventListener('input', function () {
        ...
        document.getElementById('sim-art-alpha-val').textContent = this.value + '%';
        renderSim();
      });
    }
  ...
  function renderSim() {
    ...
    ctx.beginPath();
    ctx.arc(cW * 0.5, cH * 0.40, cW * 0.16, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.ellipse(cW * 0.5, cH * 0.75, cW * 0.32, cH * 0.28, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
    ...
    if (simState.social && simState.social !== 'none') {
      ...
      if (simState.social === 'tiktok' || simState.social === 'reels' || simState.social === 'shorts') {
        ...
        for (var i = 0; i < 4; i++) {
          ...
          ctx.beginPath();
          ctx.arc(cW - rightW / 2, cH * 0.42 + i * (cH * 0.10), 10, 0, Math.PI * 2);
          ctx.fill();
        }
    ...
    if (lblPlat) lblPlat.textContent = (simState.social === 'none' ? 'Clean Frame' : simState.social.toUpperCase() + ' Overlay');
  }

  /* ---------- tabs de instalação (windows / mac / zxp) ---------- */

  function initInstallTabs() {
    var tabBtns = document.querySelectorAll('.install-tabs .tab-btn');
    ...
    faqItems.forEach(function (item) {
      ...
      if (qBtn) {
        qBtn.addEventListener('click', function () {
          ...
          faqItems.forEach(function (f) { f.classList.remove('is-open'); });
